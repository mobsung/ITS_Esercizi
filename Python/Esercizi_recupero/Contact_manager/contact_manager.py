...
Sostituisce un numero di telefono con un altro nel contatto specificato. Restituisce un nuovo dizionario con il solo contatto
...
class ContactManager:
    ...
    def update_phone_number(self, contact_name: str, old_phone_number: str, new_phone_number: str) -> dict[str, list[str]] | str:
        if contact_name in self.contacts:
            if old_phone_number in self.contacts[contact_name]:
                old_phone_number_index: str = self.contacts[contact_name].index(old_phone_number)
                self.contacts[contact_name][old_phone_number_index], new_phone_number = new_phone_number, self.contacts[contact_name][old_phone_number_index]
                return {contact_name: self.contacts[contact_name]}
            else:
                return 'Errore: il numero di telefono non è presente.'
        else:
    ...
    def list_contacts(self) -> list[str]:
        return [keys for keys in self.contacts.keys()]
    ...
    def search_contact_by_phone_number(self, phone_number: str) -> list[str] | str:
        result_list: list[str] = [contact for contact in self.contacts if phone_number in contact]
        if len(result_list) > 0:
            return result_list
        else:
            return 'Nessun contatto trovato co questo numero di telefono.'