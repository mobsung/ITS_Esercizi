...
class Documento:
    ...
    def isInText(self, text: str) -> bool:
        if text:
            return True if re.search(rf'{text.lower()}', self._text.lower()) != None else False
...
class Email(Documento):
    ...
    def __init__(self, text: str, destinatario: str, mittente: str, titolo: str) -> None:
        super().__init__(text)
    ...
    def getText(self) -> str:
        formated_text: str = f'Da: {self.mittente()}, A: {self.destinatario()}\nTitolo: {self.titolo()}\nMessaggio: {self._text}'
...
if __name__ == '__main__':
    ...
    em: Email = Email(text='Ciao, blablabla', destinatario='bla@bla.com', mittente='waza@waza.com', titolo='questa')
    
    em.writeToFile()

    print(em.isInText('ciao'))