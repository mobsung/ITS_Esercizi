...
def baricentro(v: list[int]) -> int | None:
    ...
    for i in range(len(v)):
        if sum(v[:i+1]) == sum(v[i+1:]):
            print(f'Il numero di interazioni è {(i + 1) * 2}')
            return i
...
def printResult(v: list[int]) -> None:
    if baricentro(v) != None:
        print(f'Il baricentro si trova all\'indice: {baricentro(v)}')
    else:
        print('Non eseiste un baricentro per la lista fornito')
...
def baricentroOttimizzato(v: list[int]) -> bool:
    ...
    tot: int = sum(v)
    for i in range(len(v)):
        if tot - sum(v[:i+1]) == tot // 2:
            print(f'Il numero di interazioni è {i + 1}')
            return True
    return False

if __name__ == '__main__':

    # printResult([1, 3, 3, 3, 3, 1])
    # print(baricentroOttimizzato([1, 3, 3, 3, 3, 1]))

    l_ord: list[int] = [i for i in range(10001)]
    l_rev: list[int] = [i for i in range(10000, -1, -1)]

    with Timer():

        print(baricentro(l_ord + l_rev))
    
    with Timer():

        print(baricentroOttimizzato(l_ord + l_rev))